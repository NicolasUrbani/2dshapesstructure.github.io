...
function [ affinity_matrix_with, affinity_matrix_without, n ] = computeAffinityMatrix(idSelection)
    ...
    if ~isempty(index)
        ...
        annot_shape = [];
        for ind_part = 1:n
            annot_shape = [annot_shape ; partsannotation(partsannotation(:,3)==partsId(ind_part),:)];
        end
        
        % Filter out bad users
        good_users = 1:length(teams);
        annot_shape = annot_shape(ismember(annot_shape(:,1),good_users),:);       
        
        annotations_with = annot_shape(annot_shape(:,2)==1,:);
        annotations_without = annot_shape(annot_shape(:,2)==0,:);
        ...
        for j=1:size(partsId,1)
            ...
            %partsannotationwithout_j = [cell2mat(partsannotationwithout_j(:,1)), cell2mat(partsannotationwithout_j(:,3:5))];
